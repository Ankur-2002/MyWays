...
import { Editblog, DeleteBlog } from '../Store/Action';
...
export const Blog = props => {
  ...
  const FetchBlog = async () => {
    const data = await Fetch(`/blogs/FetchBlog?id=${param.id}`, 'GET');
    // console.log(data);
    const { blog } = data;
    setuserBlog(user._id === blog[0].owner);
    setData(data.blog[0]);
  };
  ...
  const Delete = async () => {
    const data = await Fetch(`/blogs/DeleteBlog?id=${param.id}`, 'DELETE');
    if (data._id) {
